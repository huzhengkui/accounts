...
@module Chakera:accounts
*/

/**
The accounts collection, with some okc additions.

@class OkcAccounts
@constructor
*/
var collection = new Mongo.Collection("chakera_accounts", {
  connection: null
});
OkcAccounts = _.clone(collection);
OkcAccounts._collection = collection;

if (typeof PersistentMinimongo !== "undefined")
  new PersistentMinimongo(OkcAccounts._collection);
...
*/
OkcAccounts._watchBalance = function() {
  var _this = this;
  ...
  this.blockSubscription = web3.okc
...
*/
OkcAccounts._updateBalance = function() {
  var _this = this;

  _.each(OkcAccounts.find({}).fetch(), function(account) {
    web3.okc.getBalance(account.address, function(err, res) {
      if (!err) {
        if (res.toFixed) {
          res = res.toFixed();
        }

        OkcAccounts.update(account._id, {
          $set: {
            balance: res
          }
        });
...
@method _addAccounts
*/
OkcAccounts._addAccounts = function() {
  var _this = this;

  // UPDATE normal accounts on start
  web3.okc.getAccounts(function(e, accounts) {
    if (!e) {
      var visibleAccounts = _.pluck(OkcAccounts.find().fetch(), "address");
      ...
      var localAccounts = OkcAccounts.findAll().fetch();
      ...
      _.each(localAccounts, function(account) {
        ...
        if (!_.contains(accounts, account.address)) {
          OkcAccounts.updateAll(account._id, {
            $set: {
              deactivated: true
            }
          });
      ...
      _.each(accounts, function(address) {
        web3.okc.getBalance(address, function(e, balance) {
          if (!e) {
            if (balance.toFixed) {
              balance = balance.toFixed();
            }

            web3.okc.getCoinbase(function(error, coinbase) {
              if (error) {
                console.warn("getCoinbase error: ", error);
                coinbase = null; // continue with null coinbase
              }

              var doc = OkcAccounts.findAll({
                address: address
              }).fetch()[0];
              ...
              if (doc) {
                OkcAccounts.updateAll(doc._id, {
                  $set: insert
                });
              } else {
                OkcAccounts.insert(insert);
              }
...
*/
OkcAccounts._addToQuery = function(args, options) {
  var _this = this;
...
@return {Object} cursor
*/
OkcAccounts.updateAll = function() {
...
@method init
*/
OkcAccounts.init = function() {
  var _this = this;

  if (typeof web3 === "undefined") {
    console.warn(
      "OkcAccounts couldn't find web3, please make sure to instantiate a web3 object before calling OkcAccounts.init()"
    );
    return;
  }
